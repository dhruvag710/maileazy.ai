...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    try {
      // Step 1: Drop existing category_new column if it exists
      try {
        await queryInterface.removeColumn('Emails', 'category_new');
      } catch (error) {
      ...
      // Step 3: Update the data in the new column
      ...
      // Step 4: Drop the old category column and its enum
  ...
  down: async (queryInterface, Sequelize) => {
    try {
      // Step 1: Drop existing category_old column if it exists
      try {
        await queryInterface.removeColumn('Emails', 'category_old');
      } catch (error) {
        // Column doesn't exist, which is fine
      }

      // Step 2: Create a temporary column with the old enum
      ...
      // Step 3: Update the data back to old categories
      await queryInterface.sequelize.query(`
        UPDATE Emails
      ...
      // Step 4: Drop the new category column and its enum
      await queryInterface.removeColumn('Emails', 'category');

      // Step 5: Rename the old column back to category
